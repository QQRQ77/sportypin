      ...
      <div className="w-full mb-2 flex flex-col justify-center items-right">
        {rules.map((rule, index) => (
          <div key={index} className={`mb-6 ${index < rules.length - 1 ? 'border-b border-gray-300' : ''} pb-4`}>
            ...
            <h2 className="text-lg">Czas gry: <span className="font-bold">{rule.periods} </span><span>{rule.periods == 1 ? 'część' : ''}{rule.periods == 2 ? 'połowy' : ''}{rule.periods == 3 ? 'tercje' : ''}{rule.periods == 4 ? 'kwarty' : ''}{rule.periods && rule.periods > 4 ? 'części' : ''}</span>
            ...
            <>
              <h2 className="text-lg">Czas dla drużyny (przerwa w grze): <span className="font-bold">{rule.numOfTeamBreaks}</span>{(rule.numOfTeamBreaks && rule.numOfTeamBreaks > 0) ? <> na <span className="font-bold">{rule.selectedPeriodForTeamBreak}</span> x <span className="font-bold">{rule.teamBreaksSeconds}</span> sek.</> : ""}</h2>
            </>
