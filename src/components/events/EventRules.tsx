      ...
      <div className="w-full mb-2 flex flex-col justify-center items-right">
        {rules.map((rule, index) => (
          <div key={index} className="">
            ...
            x <span className="font-bold">{rule.periodMinutes}</span>; przerwa: <span className="font-bold">{rule.breakMinutes || 10} min.</span></h2>
            ...
            <h2 className="text-lg">{rule.draw_rules || "W przypadku remisu, decyduje: dogrywka (2 x 5 minut), a jeśli nadal jest remis: rzuty karne"}</h2>
            ...
            <div className="w-full ml-10 gap-2 flex flex-col justify-center items-left">
              {rule.penaltyTimeSeconds && (
                <div><span className="text-lg">wykluczenie czasowe</span> - {rule.penaltyTimeSeconds ? (rule.penaltyTimeSeconds / 60).toFixed(1) : 2} min.,</div>
              )}
              {rule.penalties && 
