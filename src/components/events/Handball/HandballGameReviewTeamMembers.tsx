          ...
          <ul className="w-full">
            {team_1_members.map((member) => (
              <li key={member.id} className="text-lg font-medium flex flex-row items-start justify-start gap-2">
                <div className="w-72 flex flex-row justify-center items-center">{member.name} <IoShirtOutline size={18} /> {member.start_number}</div>
        ...
        {team_2_members && team_2_members.length > 0 ? (
          <ul className="w-full">
            {team_2_members.map((member) => (
              <li key={member.id} className="text-lg font-medium flex flex-row items-start justify-start gap-2">
                <div className="w-72 flex flex-row justify-start items-center">{member.name} <IoShirtOutline size={18} /> {member.start_number}</div>
                <HandBallPlayerStatsDisplay
                  displayDirection="row"
                  goals={member.goals}
                  penalties={member.penalties}
