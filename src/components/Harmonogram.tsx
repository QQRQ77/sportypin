    ...
    <>
      ...
      {items.map((item, idx) => (
        <div key={item.id} className="w-full flex flex-col gap-2 mb-4">
          ...
          <div className={`lg:h-16 w-full flex flex-wrap gap-2 rounded-xl shadow-xl ${idx % 2 === 0 ? "bg-sky-200" : "bg-gray-200"} items-center ${!dateCheck(item) ? "opacity-50" : ""}`}>
                ...
                <TooltipTrigger asChild>
                  <button
                    onClick={e => {
                      e.preventDefault();
                      e.stopPropagation();
                      router.push(`/events/${eventId}/match/${matchSport}/review/${item.id}?item_LP=${item.LP}`);
                      }}
